...
string  g_sScript="TempOwner_";                              // part of script name used for settings
...
default {
    ...
    touch_start(integer num_detected)
    {
        ...
        llMessageLinked(LINK_SET,0,"TempOwner~"+llDetectedName(0)+"~"+(string)llDetectedKey(0),llDetectedKey(0));
    }
...
//                llOwnerSay(sName + " " +(string)kKey);
            ...
            if (kID == g_kMenuID) {
                ...
                if (sMessage == UPMENU) {
                    ...
                    if (kAv == g_kWearer && g_iCaptureOn) {
                        Notify(llList2Key(g_lRequests,0),llList2String(g_lRequests_Names,0)+" has captured "+llKey2Name(kAv),TRUE);
                    ...
                    UserCommand(iAuth,"capture reset",kAv,FALSE);
                    UserCommand(iAuth,"capture off",kAv,TRUE);
                }
                else if (sMessage == "ON") {
                    UserCommand(iAuth,"capture on",kAv,TRUE);
               }
