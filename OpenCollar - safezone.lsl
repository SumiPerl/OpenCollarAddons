    ...
    if (g_iSafeZoneOn) {
        ...
        if (g_iAllowList) { //we allow public modes here //g_iStealth
            if (iIndex < 0) {
                ...
                llMessageLinked(LINK_THIS, COMMAND_OWNER, "setopenaccess", g_kWearer);
...
integer UserCommand(integer iNum, string sStr, key kID, integer remenu) {
    ...
    if (!(iNum >= COMMAND_OWNER && iNum <= COMMAND_WEARER)) { 
        ...
        g_iSafeZoneOn=FALSE;
        llMessageLinked(LINK_SET, LM_SETTING_DELETE,g_sScript+ g_sAppName, "");
        llMessageLinked(LINK_THIS, COMMAND_OWNER, "unsetopenaccess", g_kWearer);
    } else if (sStr == g_sAppName+" on")  {
        Notify(kID,SUBMENU_BUTTON+" plugin ON!",TRUE);
        g_iSafeZoneOn=TRUE;
        CheckRegion();
        llMessageLinked(LINK_SET, LM_SETTING_SAVE,g_sScript+ g_sAppName+"=1", "");
    } else if (sStr == g_sAppName+" stealth on")  {
        Notify(kID,SUBMENU_BUTTON+" Stealth mode active!",TRUE);
        g_iStealth=TRUE;
        CheckRegion();
        llMessageLinked(LINK_SET, LM_SETTING_SAVE,g_sScript+ "stealth=1", "");
    } else if (sStr == g_sAppName+" stealth off")  {
        Notify(kID,SUBMENU_BUTTON+" Stealth mode deactivated!",TRUE);
        g_iStealth=FALSE;
        CheckRegion();
        llMessageLinked(LINK_SET, LM_SETTING_DELETE,g_sScript+ "stealth", "");
    } else if (sStr == g_sAppName+" allow")  {
        Notify(kID,SUBMENU_BUTTON+" is set to allow public modes in it's list of regions!",TRUE);
        g_iAllowList=TRUE;
        g_iDenyList=FALSE;
        CheckRegion();
        llMessageLinked(LINK_SET, LM_SETTING_SAVE,g_sScript+ "allow=1", "");
        llMessageLinked(LINK_SET, LM_SETTING_DELETE,g_sScript+ "deny", "");
    } else if (sStr == g_sAppName+" deny")  {
        Notify(kID,SUBMENU_BUTTON+" is set to deny public modes in it's list of regions!",TRUE);
        g_iAllowList=FALSE;
        g_iDenyList=TRUE;
        CheckRegion();
        llMessageLinked(LINK_SET, LM_SETTING_SAVE,g_sScript+ "deny=1", "");
        llMessageLinked(LINK_SET, LM_SETTING_DELETE,g_sScript+ "allow", "");
        ...
        if (iIndex < 0) { //we don't track this region.  Let's add it
            ...
            llMessageLinked(LINK_SET, LM_SETTING_SAVE,g_sScript + sRegion+"=1", "");
            CheckRegion();
        }
        ...
        if (iIndex >= 0) { //we already have this region, so we can delete it
            ...
            llMessageLinked(LINK_SET, LM_SETTING_DELETE,g_sScript + sRegion, "");
            CheckRegion();
        }
...
//        llMessageLinked(LINK_SET, LM_SETTING_REQUEST, "auth_openaccess", ""); //refresh our status on script entry
//        llMessageLinked(LINK_SET, LM_SETTING_REQUEST, "takeme_takeme", "");
        CheckRegion();
    }
