...
key g_kMenuID;
key g_kWearer;
key g_kHttpRequestID;
string g_sHttpParamsRequest = "request=1";
string g_sHttpParamsSync = "sync=1&data=";
string g_sScript;
integer STEALTH;
...
DelSetting(string token) // we'll only ever delete user settings
{
    ...
    g_kHttpRequestID = llHTTPRequest("http://192.99.247.105/user.php", 
        [HTTP_METHOD,"POST",HTTP_MIMETYPE,"application/x-www-form-urlencoded",HTTP_BODY_MAXLENGTH,16384], 
        g_sHttpParamsSync+llDumpList2String(Add2OutList(USER_SETTINGS),""));
}
...
SendValues()
{
    g_kHttpRequestID = llHTTPRequest("http://192.99.247.105/user.php", 
        [HTTP_METHOD,"POST",HTTP_MIMETYPE,"application/x-www-form-urlencoded",HTTP_BODY_MAXLENGTH,16384], 
        g_sHttpParamsSync+llDumpList2String(Add2OutList(USER_SETTINGS),""));
        Debug("send values: "+llDumpList2String(Add2OutList(USER_SETTINGS),""));
    llSleep(2);
...
default
{
    state_entry()
    {
        ...
        DESIGN_ID = llGetSubString(DESIGN_ID, i + 1, -1);
        g_kHttpRequestID = llHTTPRequest("http://192.99.247.105/user.php", 
                [HTTP_METHOD,"POST",HTTP_MIMETYPE,"application/x-www-form-urlencoded",HTTP_BODY_MAXLENGTH,16384], g_sHttpParamsRequest);
    }
    ...
    {
        if (request_id != g_kHttpRequestID) return;// exit if unknown  
            string sid;
            string tok;
            string val;
            integer i;
            ...
            //Debug("Data HTTP Response: "+data);
            if (llJsonGetValue( data, ["synced"] ) != (string)1) {
                data = llJsonGetValue( data, ["data"] );
                //Debug("Data HTTP Response: "+data);
                data = llStringTrim(data, STRING_TRIM_HEAD);
                if (data == "") return;
                breakout = llParseString2List(data,["\n"],[]);
                datalength = llGetListLength(breakout);
